...
class DB_SCAN:
    ...
    def Get_distanceMean(self, points, minPts, previous_distanceMean, metryka):
        """
        ...
        if (minPts < len(points)):
            nbrs = NearestNeighbors(n_neighbors=minPts, metric=metryka).fit(points)
    ...
    def KNNdist_plot(self, points, minPts,metryka):
        """
       ...
       """
        ...
        for i in tqdm(range(0, len(points), 5), desc= "Liczenie eps", dynamic_ncols=True):
            current_distanceMean = self.Get_distanceMean(points[i:], minPts, previous_distanceMean, metryka)
